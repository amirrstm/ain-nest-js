...
> extends DatabaseBaseRepositoryAbstract<EntityDocument> {
  ...
  async findAll<T = EntityDocument>(
    ...
    if (options?.join) {
      findAll.populate(
        typeof options.join === 'boolean' ? this._joinOnFind : (options.join as PopulateOptions | PopulateOptions[])
      )
    }
  ...
  async createMany<Dto>(data: Dto[], options?: IDatabaseCreateManyOptions<ClientSession>): Promise<boolean> {
    const dataCreate: Record<string, any>[] = data.map((val: Record<string, any>) => ({
      ...val,

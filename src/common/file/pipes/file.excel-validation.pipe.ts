...
import { IFileExtract, IFileExtractAllSheets } from 'src/common/file/interfaces/file.interface'
...
export class FileExcelValidationPipe<T = any> implements PipeTransform {
  ...
  async transformMerge(value: IFileExtract<T>, classTransform: T[]): Promise<IFileExtract<T>> {
  ...
  async validate(value: IFileExtract<T>): Promise<void> {
    if (
      ![ENUM_FILE_MIME.CSV, ENUM_FILE_MIME.XLS, ENUM_FILE_MIME.XLSX].find(val => val === value.mimetype.toLowerCase())
    ) {
  ...
  async validateExtract(classTransform: T[], filename: string, sheet: number): Promise<void> {
...
@Injectable()
export class FileExcelValidationAllSheetPipe<T = any[]> implements PipeTransform {
  ...
  async transform(
    ...
    if (Array.isArray(value)) {
      for (let val of value) {
        ...
        for (let i = 0; i < (this.dto as Array<T>).length; i++) {
          const classTransform: T[] = await this.transformExtract(this.dto[i] as ClassConstructor<any>, val.extracts[i])
    ...
    for (let i = 0; i < (this.dto as Array<T>).length; i++) {
      const classTransform: T[] = await this.transformExtract(this.dto[i] as ClassConstructor<any>, value.extracts[i])
  ...
  async validate(value: IFileExtractAllSheets<T>): Promise<void> {
    if (
      ...
      throw new BadRequestException({
        statusCode: ENUM_FILE_STATUS_CODE_ERROR.FILE_VALIDATION_ALL_SHEET_DTO_ERROR,

...
describe('PaginationService', () => {
  ...
  describe('order', () => {
    it('should return the order as a key-value pair', () => {
      expect(service.order('title', ENUM_PAGINATION_ORDER_DIRECTION_TYPE.ASC, ['title', 'createdAt'])).toEqual({
        title: ENUM_PAGINATION_ORDER_DIRECTION_TYPE.ASC,
      })
    })

    it('should use the default orderBy, because of key-value not pair', () => {
      expect(service.order('name', ENUM_PAGINATION_ORDER_DIRECTION_TYPE.ASC, ['title', 'createdAt'])).toEqual({
  ...
  describe('search', () => {
    ...
    it('should return undefined if no searchValue is provided, with undefined', () => {
      expect(service.search(undefined, ['firstName', 'lastName'])).toBeUndefined()
    })

    it('should return undefined if no searchValue is provided, with empty string', () => {
      expect(service.search('', ['firstName', 'lastName'])).toBeUndefined()
    })
  })

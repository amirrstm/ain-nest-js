...
export class HelperDateService implements IHelperDateService {
  ...
  forwardInMilliseconds(milliseconds: number, options?: IHelperDateOptionsForward): Date {
    return moment(options?.fromDate).tz(this.defTz).add(milliseconds, 'ms').toDate()
  }

  backwardInMilliseconds(milliseconds: number, options?: IHelperDateOptionsBackward): Date {
    return moment(options?.fromDate).tz(this.defTz).subtract(milliseconds, 'ms').toDate()
  }

  forwardInSeconds(seconds: number, options?: IHelperDateOptionsForward): Date {
    return moment(options?.fromDate).tz(this.defTz).add(seconds, 's').toDate()
  }

  backwardInSeconds(seconds: number, options?: IHelperDateOptionsBackward): Date {
    return moment(options?.fromDate).tz(this.defTz).subtract(seconds, 's').toDate()
  }

  forwardInMinutes(minutes: number, options?: IHelperDateOptionsForward): Date {
    return moment(options?.fromDate).tz(this.defTz).add(minutes, 'm').toDate()
  }

  backwardInMinutes(minutes: number, options?: IHelperDateOptionsBackward): Date {
    return moment(options?.fromDate).tz(this.defTz).subtract(minutes, 'm').toDate()
  }

  forwardInHours(hours: number, options?: IHelperDateOptionsForward): Date {
    return moment(options?.fromDate).tz(this.defTz).add(hours, 'h').toDate()
  }

  backwardInHours(hours: number, options?: IHelperDateOptionsBackward): Date {
    return moment(options?.fromDate).tz(this.defTz).subtract(hours, 'h').toDate()
  }
  ...
  backwardInDays(days: number, options?: IHelperDateOptionsBackward): Date {
    return moment(options?.fromDate).tz(this.defTz).subtract(days, 'd').toDate()
  }

  forwardInMonths(months: number, options?: IHelperDateOptionsForward): Date {
    return moment(options?.fromDate).tz(this.defTz).add(months, 'M').toDate()
  }

  backwardInMonths(months: number, options?: IHelperDateOptionsBackward): Date {
    return moment(options?.fromDate).tz(this.defTz).subtract(months, 'M').toDate()
  }
  ...
  setTime(date: Date, { hour, minute, second }: IHelperDateSetTimeOptions): Date {
    return moment(date)
      .tz(this.defTz)
      .set({
        hour: hour,
  ...
  addTime(date: Date, { hour, minute, second }: IHelperDateSetTimeOptions): Date {
    return moment(date)
      .tz(this.defTz)
      .add({
        hour: hour,
  ...
  minusTime(date: Date, { hour, minute, second }: IHelperDateSetTimeOptions): Date {
    return moment(date)
      .tz(this.defTz)
      .subtract({
        hour: hour,

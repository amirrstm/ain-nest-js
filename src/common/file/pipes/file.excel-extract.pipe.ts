...
import { IFile, IFileExtract, IFileExtractAllSheets } from 'src/common/file/interfaces/file.interface'
...
export class FileExcelExtractPipe<T> implements PipeTransform {
  ...
  async transform(value: IFile | IFile[]): Promise<IFileExtract<T> | IFileExtract<T>[]> {
    if (!value) {
      return
    }
  ...
  async validate(mimetype: string): Promise<void> {
    if (![ENUM_FILE_MIME.CSV, ENUM_FILE_MIME.XLS, ENUM_FILE_MIME.XLSX].find(val => val === mimetype.toLowerCase())) {
  ...
  async extract(value: IFile): Promise<IFileExtract<T>> {
    const extracts: IHelperFileRows[][] = this.helperFileService.readExcelFromBuffer(value.buffer, {
      sheet: 0,
    })
...
export class FileExcelExtractAllSheetPipe<T> implements PipeTransform {
  ...
  async transform(value: IFile | IFile[]): Promise<IFileExtractAllSheets<T> | IFileExtractAllSheets<T>[]> {
    if (!value) {
      return
    }
